...
class ChessConfig(BaseGameConfig):
    ...
    white_player_name: str | None = Field(default=None, description='Name of the white player')
    black_player_name: str | None = Field(default=None, description='Name of the black player')
    ...
    recursion_limit: int = Field(default=600, description='Python recursion limit for chess')
    engines: list[Any] | None = Field(default=None, description='LLM configurations for players and analyzers')
    ...
    def get_role_definitions(self) -> dict[str, GamePlayerRole]:
        ...
        return {'white_player': GamePlayerRole(name='white_player', display_name=self.white_player_name or 'White', default_model='gpt-4o'), 'black_player': GamePlayerRole(name='black_player', display_name=self.black_player_name or 'Black', default_model='claude-3-5-sonnet-20240620'), 'white_analyzer': GamePlayerRole(name='white_analyzer', display_name='White Analyst', is_analyzer=True, default_model='gpt-4o'), 'black_analyzer': GamePlayerRole(name='black_analyzer', display_name='Black Analyst', is_analyzer=True, default_model='claude-3-5-sonnet-20240620')}

    def get_example_configs(self) -> dict[str, dict[str, Any]]:
        """Define example chess configurations."""
        return {'gpt_vs_claude': {'white_model': 'gpt-4o', 'black_model': 'claude-3-5-sonnet-20240620', 'white_player_name': 'GPT White', 'black_player_name': 'Claude Black'}, 'budget': {'white_model': 'gpt-3.5-turbo', 'black_model': 'claude-3-haiku-20240307', 'temperature': 0.5}}
    ...
    def create_simple_player_configs(self) -> dict[str, PlayerAgentConfig]:
        ...
        black_model = self.black_model or self.player2_model or 'claude-3-5-sonnet-20240620'
        return {'white_player': PlayerAgentConfig(llm_config=white_model, temperature=self.temperature, player_name=self.white_player_name or 'White'), 'black_player': PlayerAgentConfig(llm_config=black_model, temperature=self.temperature, player_name=self.black_player_name or 'Black'), 'white_analyzer': PlayerAgentConfig(llm_config=white_model, temperature=0.2, player_name='White Analyst'), 'black_analyzer': PlayerAgentConfig(llm_config=black_model, temperature=0.2, player_name='Black Analyst')}
    ...
    def finalize_config(self) -> 'ChessConfig':
        ...
        if not self.white_player_name and self.engines:
            for engine in self.engines:
                if hasattr(engine, 'prompt_template') and 'white' in str(engine.prompt_template):
                    self.white_player_name = 'White'
                    break
        if not self.black_player_name and self.engines:
            for engine in self.engines:
                if hasattr(engine, 'prompt_template') and 'black' in str(engine.prompt_template):
