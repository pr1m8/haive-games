...
from haive.games.nim.models import (
    NimAnalysis,
    NimMove,
)
...
class NimPromptGenerator(GenericPromptGenerator[str, str]):
    ...
    def create_move_prompt(self, player: str) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    f"You are {player} in a Nim game. You are playing Nim. Your goal is to force your opponent to take the last stone (or take it yourself, depending on the variant).\n\n"
                    "As a Nim player:\n"
                    "- Calculate nim-sum to determine winning positions"
                    "- Force opponent into losing positions"
                    "- Understand the mathematical foundation of the game"
                    "- Plan endgame scenarios carefully"
                    "- Use optimal play theory\n\n"
                    "Key strategies:\n"
    ...
    def create_analyzer_prompt(self, player: str) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    f"You are an expert Nim strategist analyzing the current game state for {player}.\n\n"
...
class NimEngineFactory(GenericGameEngineFactory[str, str]):
...
def create_generic_nim_engines(
    ...
    Returns:
        Dict[str, AugLLMConfig]: Dictionary of Nim engines
    ...
    """
    return nim_factory.create_engines(player_configs)
...
def create_generic_nim_engines_simple(
    ...
    return create_engines_from_simple_configs(
        nim_factory, player1_model, player2_model, temperature
    )
...
def create_generic_nim_config_from_example(
    ...
    return create_generic_nim_engines_simple(player1_model, player2_model, temperature)
