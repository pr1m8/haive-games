...
from ...card.components.actions import CardAction, ActionResult
from ...card.components.betting import BetType, BettingAction
from ...card.components.standard import StandardCard
from .state import PokerGameState, PokerPhase
...
class RaiseAction(PokerAction):
    ...
    def can_execute(self, state: PokerGameState) -> bool:
        ...
        # Check if player has enough chips
        return (
            self.amount >= min_raise and
            state.player_chips.get(self.player_id, 0) >= (self.amount - current_player_bet)
        )
...
class FoldAction(PokerAction):
    ...
    def execute(self, state: PokerGameState) -> ActionResult:
        ...
        if not self.can_execute(state):
            return ActionResult(
                success=False,
                message="Cannot fold: not your turn"
            )
...
class AllInAction(PokerAction):
    ...
    def execute(self, state: PokerGameState) -> ActionResult:
        ...
        if not self.can_execute(state):
            return ActionResult(
                success=False,
                message="Cannot go all-in: not your turn"
            )
