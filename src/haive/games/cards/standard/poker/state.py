...
from ...card.components.state import CardGameState
from ...card.components.actions import CardAction
from ...card.components.standard import StandardCard
from ...card.components.betting import WagerableGameState
from ...card.components.turns import TurnPhase
from ...card.components.rules import RuleSet
from .scoring import PokerHandEvaluator, PokerHandRank
...
class PokerGameState(CardGameState[StandardCard, CardAction], WagerableGameState):
    ...
    def _distribute_winnings(self) -> None:
        ...
        winners = [player_id for player_id, rank in self.hand_rankings.items() 
                  if rank == best_rank]
    ...
    def get_player_view(self, player_id: str) -> Dict[str, Any]:
        ...
        view = {
            "my_hand": [card.dict() for card in self.hands[player_id].cards] if player_id in self.hands else [],
            ...
            "dealer": self.players[self.dealer_position] if self.dealer_position < len(self.players) else None,
            "current_player": self.current_player_id,
            "phase": self.phase,
            "hand_rankings": {pid: str(rank) for pid, rank in self.hand_rankings.items()} if self.phase == PokerPhase.SHOWDOWN else {},
