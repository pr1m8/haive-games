...
class BlackjackAgentConfig(AgentConfig):
    ...
    initial_chips: float = Field(
        default=1000.0, description="Starting chip amount for each player"
    )
    state_schema: type[BaseModel] = Field(
        default=BlackjackGameState, description="State schema for the Blackjack game"
    )
