...
This module provides configurable Poker game configurations that replace
...
from haive.games.poker.generic_engines import (
...
class ConfigurablePokerConfig(PokerAgentConfig):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.player_configs:
            ...
            self.engines = create_generic_poker_engines(self.player_configs)
            self._extract_player_names_from_configs()

        elif self.example_config:
            # Method 2: Use example configuration
            self.engines = create_generic_poker_config_from_example(
            ...
            self.engines = create_generic_poker_engines_simple(
    ...
    def _generate_player_names_from_example(self):
        ...
        example_names = {
            "gpt_vs_claude": ("GPT Poker Pro", "Claude Poker Ace"),
            "gpt_only": ("GPT Poker Pro", "GPT Poker Ace"),
            "claude_only": ("Claude Poker Pro", "Claude Poker Ace"),
            "budget": ("Budget Poker Pro", "Budget Poker Ace"),
            "mixed": ("Mixed Poker Pro", "Mixed Poker Ace"),
            "advanced": ("Advanced Poker Pro", "Advanced Poker Ace"),
        }

        if self.example_config in example_names:
            self.player1_name, self.player2_name = example_names[self.example_config]
        else:
            self.player1_name = f"{self.example_config} Poker Pro"
            self.player2_name = f"{self.example_config} Poker Ace"
    ...
    def _generate_player_names_from_models(
        ...
        self.player1_name = (
            ...
            or f"{model_to_name(player1_model)} Poker Pro"
        )
        self.player2_name = (
            getattr(self, "player2_name", None)
            or f"{model_to_name(player2_model)} Poker Ace"
        )


def create_poker_config(
    ...
    **kwargs,
    ...
    Example:
        ...
        ... )
    """
    return ConfigurablePokerConfig(
        player1_model=player1_model, player2_model=player2_model, **kwargs
    )


def create_poker_config_from_example(
    ...
    """
    return ConfigurablePokerConfig(example_config=example_name, **kwargs)
...
def create_poker_config_from_player_configs(
    ...
    """
    return ConfigurablePokerConfig(player_configs=player_configs, **kwargs)


# Convenience functions for common configurations


def create_budget_poker_config(**kwargs) -> ConfigurablePokerConfig:
    """Create a budget-friendly Poker configuration."""
    return create_poker_config_from_example("budget", **kwargs)


def create_advanced_poker_config(**kwargs) -> ConfigurablePokerConfig:
    """Create an advanced Poker configuration with powerful models."""
    return create_poker_config_from_example("advanced", **kwargs)


def create_experimental_poker_config(**kwargs) -> ConfigurablePokerConfig:
    """Create an experimental Poker configuration with mixed providers."""
    return create_poker_config_from_example("mixed", **kwargs)
...
EXAMPLE_CONFIGURATIONS = {
    "simple": {
        ...
        "config": lambda: create_poker_config("gpt-4o", "claude-3-opus"),
    },
    "budget": {
        "description": "Cost-effective configuration",
        "config": lambda: create_budget_poker_config(enable_analysis=False),
    },
    "advanced": {
        "description": "High-performance strategic setup",
        "config": lambda: create_advanced_poker_config(temperature=0.2),
    },
    "experimental": {
        "description": "Mixed providers and settings",
        "config": lambda: create_experimental_poker_config(temperature=0.5),
    },
}


def get_example_config(name: str) -> ConfigurablePokerConfig:
    ...
    Returns:
        ConfigurablePokerConfig: The example configuration
...
if __name__ == "__main__":
    ...
    print("🎮 Configurable Poker Configuration Demo")
    print("=" * 50)

    # Example 1: Simple configuration
    print("1️⃣ Simple Configuration:")
    config1 = create_poker_config("gpt-4o", "claude-3-opus")
    print(f"   Poker Pro: {config1.player1_name}")
    print(f"   Poker Ace: {config1.player2_name}")
    print(f"   Engines: {len(config1.engines)}")
    print()

    # Example 2: Example configuration
    print("2️⃣ Example Configuration:")
    config2 = create_poker_config_from_example("advanced")
    print(f"   Poker Pro: {config2.player1_name}")
    print(f"   Poker Ace: {config2.player2_name}")
    ...
    player_configs = {
        "player1_player": PlayerAgentConfig(
            llm_config="gpt-4o", player_name="Master Poker Pro", temperature=0.8
        ),
        "player2_player": PlayerAgentConfig(
            llm_config="claude-3-opus", player_name="Expert Poker Ace", temperature=0.4
        ),
        "player1_analyzer": PlayerAgentConfig(
    ...
    config3 = create_poker_config_from_player_configs(player_configs)
    print(f"   Poker Pro: {config3.player1_name}")
    print(f"   Poker Ace: {config3.player2_name}")
    print()
    ...
    print("✅ Poker configurable system ready!")
