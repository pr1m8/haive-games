...
class GameDebateAgent(DebateConversation):
    ...
    """
    mode: Literal['game_debate'] = Field(default='game_debate', description='Game debate mode identifier')
    state_schema: type[BaseModel] = Field(default=DebateState, description='State schema class to use for this agent')
    scoring_enabled: bool = Field(default=True, description='Whether to enable argument scoring')
    points_per_argument: int = Field(default=10, ge=1, le=50, description='Base points awarded per argument')
    points_per_rebuttal: int = Field(default=15, ge=1, le=50, description='Base points awarded per rebuttal')
    bonus_for_evidence: int = Field(default=5, ge=0, le=20, description='Bonus points for citing evidence')
    penalty_for_repetition: int = Field(default=3, ge=0, le=10, description='Point penalty for repetitive arguments')
    tournament_mode: bool = Field(default=False, description='Whether this is a tournament match')
    match_id: str | None = Field(default=None, description='Unique identifier for tournament match')
    bracket_position: str | None = Field(default=None, description="Position in tournament bracket (e.g., 'semifinal_1')")
    track_performance: bool = Field(default=True, description='Whether to track detailed performance metrics')
    save_replay: bool = Field(default=True, description='Whether to save debate replay for analysis')
    ...
    def setup_agent(self) -> None:
        ...
        super().setup_agent()
        logger.debug(f'GameDebateAgent setup complete with state schema: {self.state_schema}')
    ...
    def _custom_initialization(self, state: DebateState) -> dict[str, Any]:
        ...
        game_fields = {'player_scores': dict.fromkeys((self.debate_positions or {}).keys(), 0), 'argument_scores': {}, 'rebuttal_scores': {}, 'bonus_points': dict.fromkeys((self.debate_positions or {}).keys(), 0), 'penalty_points': dict.fromkeys((self.debate_positions or {}).keys(), 0), 'response_times': {}, 'argument_quality': {}, 'consistency_scores': {}, 'match_metadata': {'match_id': self.match_id, 'bracket_position': self.bracket_position, 'tournament_mode': self.tournament_mode}, 'game_phase': 'starting', 'round_scores': [], 'mvp_candidate': None}
    ...
    def _create_initial_message(self) -> BaseMessage:
        ...
        if self.scoring_enabled:
            game_features.append(f'📊 Scoring: {self.points_per_argument} pts per argument, {self.points_per_rebuttal} pts per rebuttal')
            if self.bonus_for_evidence > 0:
                game_features.append(f'💎 Evidence Bonus: +{self.bonus_for_evidence} pts for citing sources')
            if self.penalty_for_repetition > 0:
                game_features.append(f'⚠️ Repetition Penalty: -{self.penalty_for_repetition} pts for redundant arguments')
        if self.tournament_mode:
            game_features.append(f'🏆 Tournament Match: {self.bracket_position or 'Competition'}')
            if self.match_id:
                game_features.append(f'🆔 Match ID: {self.match_id}')
        game_features_str = '\n'.join(game_features) if game_features else 'Standard debate rules'
        title = '🎮 Welcome to the Gamified Debate Tournament!' if self.tournament_mode else '🎮 Welcome to the Gamified Debate!'
        return HumanMessage(content=f"{title}\n\n📋 **Topic**: {self.topic}\n\n👥 **Players and Positions**:\n{positions_str}\n\n🎯 **Game Features**:\n{game_features_str}\n\n📜 **How to Win**:\n• Make compelling, well-researched arguments\n• Counter your opponent's points effectively\n• Cite evidence and sources for bonus points\n• Stay consistent with your position\n• Engage with respect and intellectual honesty\n\n🎲 **Let the game begin!** {(next(iter(positions.keys())) if positions else 'Players')}, present your opening move!")
    ...
    def process_response(self, state: DebateState) -> Command:
        ...
        try:
            ...
            updates = base_updates.update if hasattr(base_updates, 'update') and base_updates.update else {}
        ...
        if self.scoring_enabled and speaker in (self.debate_positions or {}):
            score_updates = self._calculate_argument_score(content, speaker, phase, state)
            updates.update(score_updates)
        game_phase_updates = self._update_game_phase(state)
        updates.update(game_phase_updates)
        return Command(update=updates)

    def _calculate_argument_score(self, content: str, speaker: str, phase: str, state: DebateState) -> dict[str, Any]:
        ...
        if self.penalty_for_repetition > 0 and self._is_repetitive(content, speaker, state):
        ...
        argument_scores[speaker].append({'content_preview': content[:100] + '...' if len(content) > 100 else content, 'base_score': base_score, 'bonus': bonus, 'penalty': penalty, 'final_score': final_score, 'phase': phase})
        ...
        logger.debug(f'Scored {speaker}: {final_score} pts (base: {base_score}, bonus: {bonus}, penalty: {penalty})')
        return updates

    def _has_evidence(self, content: str) -> bool:
        """Detect if content cites evidence or sources."""
        evidence_indicators = ['according to', 'research shows', 'studies indicate', 'data suggests', 'statistics show', 'source:', 'citation:', 'reference:', 'www.', 'http', '.com', '.org', '.edu', 'published in', 'journal', 'university', 'institute', 'report', 'survey']
    ...
    def _update_game_phase(self, state: DebateState) -> dict[str, Any]:
        ...
        elif current_game_phase == 'playing' and getattr(state, 'should_end_debate', False):
            updates['game_phase'] = 'scoring'
        elif current_game_phase == 'scoring' and getattr(state, 'conversation_ended', False):
    ...
    def conclude_conversation(self, state: DebateState) -> Command:
        ...
        base_updates = base_conclusion.update if hasattr(base_conclusion, 'update') else {}
        final_scores = getattr(state, 'player_scores', {})
        winner = max(final_scores.items(), key=lambda x: x[1])[0] if final_scores else None
        summary_parts = ['🏁 **GAME OVER** - Debate Tournament Match Complete!', f"📋 Topic: '{self.topic}'", '', '🏆 **FINAL SCORES:**']
        ...
        if self.tournament_mode:
            summary_parts.extend(['', '🏆 **TOURNAMENT INFO:**', f'  • Match ID: {self.match_id or 'Unknown'}', f'  • Bracket: {self.bracket_position or 'Unknown'}', f'  • Advancing Player: {winner or 'TBD'}'])
        if self.track_performance:
            summary_parts.extend(['', '📈 **PERFORMANCE STATS:**', f'  • Total Rounds: {len(getattr(state, 'phase_transitions', []))}', f'  • Arguments Made: {getattr(state, 'total_arguments', 0)}', f'  • Rebuttals Given: {getattr(state, 'total_rebuttals', 0)}'])
        conclusion_msg = SystemMessage(content='\n'.join(summary_parts))
        game_updates = {'messages': [conclusion_msg], 'conversation_ended': True, 'current_phase': 'complete', 'game_phase': 'complete', 'debate_winner': winner, 'final_scores': final_scores}
        base_updates.update(game_updates)
        return Command(update=base_updates)

    @classmethod
    def create_tournament_match(cls, topic: str, player_a: tuple[str, str], player_b: tuple[str, str], match_id: str, bracket_position: str='tournament', **kwargs) -> 'GameDebateAgent':
        """Create a tournament debate match."""
        name_a, pos_a = player_a
        name_b, pos_b = player_b
        agents = {}
        engine_a = AugLLMConfig(name=f'{name_a.lower()}_tournament_engine', system_message=f"🎮 TOURNAMENT MODE ACTIVATED 🎮\n\nYou are {name_a} competing in a formal debate tournament.\nTopic: '{topic}'\nYour Position: {pos_a}\n\nWINNING STRATEGY:\n• Make 2-3 strong, evidence-backed arguments\n• Counter opponent's points decisively\n• Cite sources for bonus points\n• Maintain logical consistency\n• Be respectful but competitive\n\nThis is a scored competition - give your best performance!", temperature=0.6)
        agents[name_a] = SimpleAgent(name=f'{name_a}_tournament_agent', engine=engine_a, state_schema=DebateState)
        engine_b = AugLLMConfig(name=f'{name_b.lower()}_tournament_engine', system_message=f"🎮 TOURNAMENT MODE ACTIVATED 🎮\n\nYou are {name_b} competing in a formal debate tournament.\nTopic: '{topic}'\nYour Position: {pos_b}\n\nWINNING STRATEGY:\n• Make 2-3 strong, evidence-backed arguments\n• Counter opponent's points decisively\n• Cite sources for bonus points\n• Maintain logical consistency\n• Be respectful but competitive\n\nThis is a scored competition - give your best performance!", temperature=0.6)
        agents[name_b] = SimpleAgent(name=f'{name_b}_tournament_agent', engine=engine_b, state_schema=DebateState)
        tournament_kwargs = {'name': f'TournamentMatch_{match_id}', 'participant_agents': agents, 'topic': topic, 'debate_positions': {name_a: pos_a, name_b: pos_b}, 'tournament_mode': True, 'match_id': match_id, 'bracket_position': bracket_position, 'arguments_per_side': 3, 'enable_opening_statements': True, 'enable_closing_statements': True, 'state_schema': DebateState}
    ...
    def __repr__(self) -> str:
        ...
        positions = ', '.join([f'{name}={pos[:15]}...' for name, pos in (self.debate_positions or {}).items()])
