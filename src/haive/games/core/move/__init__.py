...
class Game(BaseModel, Generic[B, P]):
    ...
    def _reconstruct_state(self) -> MoveResult:
        ...
        return MoveResult(
            success=True, message="Game state reconstructed", game_state_changed=True
        )
...
class SetValueMove(SudokuMove):
    ...
    def validate(self, game: "SudokuGame") -> Tuple[bool, str]:
        ...
        if not game.board.is_valid_placement(self.row, self.col, self.value):
            return (
                False,
                f"Value {self.value} is not valid at position ({self.row}, {self.col})",
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert move to dictionary representation."""
        data = super().to_dict()
        data.update({"row": self.row, "col": self.col, "value": self.value})
        return data
...
class ClearCellMove(SudokuMove):
    ...
    def execute(self, game: "SudokuGame") -> MoveResult:
        ...
        if success:
            return MoveResult(
                success=True,
                message=f"Cell cleared at position ({self.row}, {self.col})",
            )
