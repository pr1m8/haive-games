...
class CrosswordClue(BaseModel):
    ...
    def end_position(self) -> GridPosition:
        ...
        if self.direction == Direction.ACROSS:
            return GridPosition(row=self.start_position.row, col=self.start_position.col + self.length - 1)
        return GridPosition(row=self.start_position.row + self.length - 1, col=self.start_position.col)
...
class CrosswordWord(BaseModel):
    ...
    def validate_word(self) -> CrosswordWord:
        ...
        if len(self.positions) != len(self.letters) or len(self.positions) != self.clue.length:
...
class CrosswordGame(Game[GridPosition, CrosswordLetter]):
    ...
    def is_valid_move(self, move: CrosswordMove | dict[str, any]) -> bool:
        ...
        if isinstance(move, dict):
            try:
                move = CrosswordMove(position=GridPosition(row=move['position']['row'], col=move['position']['col']), letter=move['letter'])
    ...
    def check_word(self, clue_id: str) -> bool:
        ...
        return all((self.board.check_letter(pos) for _i, pos in enumerate(word.positions)))
    ...
    def reset(self) -> None:
        ...
        for space in self.board.spaces.values():
            if isinstance(space, CrosswordCell) and space.is_letter_cell and space.is_occupied():
                space.remove_piece()
...
class CrosswordTemplate(BaseModel):
    ...
    def create_game(self) -> CrosswordGame:
        ...
        for clue_data in self.clues:
            start_pos = GridPosition(row=clue_data['start_row'], col=clue_data['start_col'])
            board.add_clue(number=clue_data['number'], direction=clue_data['direction'], text=clue_data['text'], answer=clue_data['answer'], start_position=start_pos)
        return game