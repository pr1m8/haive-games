...
class DominoesPromptGenerator(GenericPromptGenerator[str, str]):
    ...
    def create_move_prompt(self, player: str) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    f"You are {player} in a Dominoes game. You are playing a domino game. Your goal is to play all your dominoes before your opponent or score the most points.\n\n"
                    "As a Dominoes player:\n"
                    "- Play dominoes that limit your opponent's options"
                    "- Keep track of which numbers have been played"
                    "- Save versatile dominoes (doubles) for strategic moments"
                    "- Block your opponent when possible"
                    "- Calculate points to determine optimal plays\n\n"
                    "Key strategies:\n"
    ...
    def create_analyzer_prompt(self, player: str) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    f"You are an expert Dominoes strategist analyzing the current game state for {player}.\n\n"
...
# Factory instance
dominoes_factory = DominoesEngineFactory()
...
def create_generic_dominoes_engines(
    ...
    Returns:
        Dict[str, AugLLMConfig]: Dictionary of Dominoes engines
    ...
    """
    return dominoes_factory.create_engines(player_configs)
...
def create_generic_dominoes_engines_simple(
    ...
    return create_engines_from_simple_configs(
        dominoes_factory, player1_model, player2_model, temperature
    )
...
def create_generic_dominoes_config_from_example(
    ...
    return create_generic_dominoes_engines_simple(
