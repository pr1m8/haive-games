...
class GridBoard(Board[GridSpace[T], GridPosition, T]):
    ...
    def get_space_at_position(self, position: GridPosition) -> Optional[GridSpace[T]]:
        ...
        for space in self.spaces.values():
            if (space.position.row == position.row and 
                space.position.col == position.col):
                return space
        return None
    ...
    def initialize_grid(self, space_factory: Optional[Callable[[int, int], GridSpace[T]]] = None) -> None:
        """
